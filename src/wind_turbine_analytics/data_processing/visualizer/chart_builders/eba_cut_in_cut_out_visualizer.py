...
class EbaCutInCutOutVisualizer(BaseVisualizer):
    """
    Visualiseur pour la disponibilité énergétique (EBA - Energy Based Availability).
    ...
    def _create_figure(self, result: AnalysisResult) -> go.Figure:
        ...
        Args:
            result: Résultat de EbACutInCutOutAnalyzer contenant:
        ...
        if not result.detailed_results:
            logger.warning("Aucune donnée détaillée pour générer le graphique EBA")
    ...
    def _create_empty_figure(self) -> go.Figure:
        ...
        fig = go.Figure()
        fig.add_annotation(
            text="No data available for EBA analysis",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
        ...
        fig.update_layout(
            title="Energy Based Availability Analysis",
            height=400,
            width=600,
            template="plotly_white",
        )
